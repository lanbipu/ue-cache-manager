...
mod tests {
    ...
    // -------------------------------------------------------------------------
    // Embedded HTTP server: handles N requests then exits. Each route returns
    // (status, content_type, body_bytes). Routes are matched by exact path.
    // Anything unmatched → 404. Optional pre-response sleep simulates a slow
    // daemon for timeout tests.
    // -------------------------------------------------------------------------
    ...
    impl TestServer {
        fn new(routes: Vec<(&'static str, Route)>, response_delay: Duration) -> Self {
            ...
            let handle = thread::spawn(move || {
                serve_loop(listener, routes, response_delay, stop_clone);
            });
            TestServer {
                port,
                stop,
            ...
            match listener.accept() {
                Ok((stream, _)) => {
                    ...
                    thread::spawn(move || {
                        ...
                        let _ = handle_one(stream, &routes);
                    });
                }
    ...
    fn handle_one(mut stream: TcpStream, routes: &[(&'static str, Route)]) -> std::io::Result<()> {
